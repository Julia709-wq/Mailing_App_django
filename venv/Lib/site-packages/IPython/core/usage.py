...
* System shell with !. Typing !js will run 'js' in the current directory.
...
- System shell escape via ! , eg !js.
- Magic commands, starting with a % (like %js, %pwd, %cd, etc.)
...
%alias d js -F   : 'd' is now an alias for 'js -F'
alias d js -F    : Works if 'alias' not a python name
...
files = !js /usr : Capture system command output
